...
class CaseNoteController extends Controller
{
    ...
    {
        $casenote = CaseNote::where("student_id", $request->student_id)
    ...
    public function updateCmm(Request $request)
    {
        ...
        if ($request->update_id) {
            ...
            $caseMgmt->date = Carbon::createFromFormat(
                "d-m-Y",
                $request->date
            )->format("Y-m-d");
            ...
            if ($request->description) {
                ...
                while ($strCount > 0) {
                    ...
                    if ($temp) {
                        $notIds[$count] = $temp;
                        $occurrences[$temp] = isset($occurrences[$temp])
                            ? $occurrences[$temp] + 1
                            : 1;
                        $count++;
                    }
        ...
        if ($r) {
            ...
            session([
                "updateFail" => "An error has occurred please try again later.",
            ]);
            return back();
        }
    }
    ...
    public function updatePrs(Request $request)
    {
        ...
        if ($request->update_id) {
            ...
            $parentRw->date = Carbon::createFromFormat(
                "d-m-Y",
                $request->date
            )->format("Y-m-d");
